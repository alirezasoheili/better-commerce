...
`OPERATIONS_DATABASE_URL`, `READINESS_DATABASE_URL`, and (when `example` is
enabled) `EXAMPLE_RUNTIME_DATABASE_URL` to the same installation database.
...
respective schema access. Keep the operations URL out of the server process;
the server uses only the two scoped URLs.
